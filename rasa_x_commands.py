...
def create_argparser():
    ...
    create_user = subparsers.add_parser('create',
                                        help='creates a new Rasa X user')
    create_user.add_argument('role',
    ...
    create_user.add_argument('password',
                             help="Password to use for the user")
    create_user.add_argument('--update',
                             action='store_true',
                             help='Update the password of an existing user')
    create_user.set_defaults(func=create_rasa_x_user)

    delete_user = subparsers.add_parser('delete',
                                        help='delete a Rasa X user')
...
def create_rasa_x_user(args):
    ...
    if args.update:
        create_cmd += " --update"
    retcode = subprocess.call('docker-compose exec rasa-x bash '
                              '-c "python {}"'.format(create_cmd),
                              shell=True)
